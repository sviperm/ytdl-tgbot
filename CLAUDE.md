...
**Instagram** is handled separately (yt-dlp/gallery-dl now need login for it). `video_link_handler` routes `instagram.com` URLs to `handle_instagram`, which uses `src/downloader/instagram.py` — a no-login fetcher: primary path is the internal `api/v1/media/<pk>/info` endpoint with the public `X-IG-App-ID` (returns video + carousel + caption; may be login-gated from datacenter IPs), falling back to the `embed/captioned` page (images + caption only, no video). The whole post is sent as a Telegram **media group** (chunked to 10) with the caption on the first item; single items are sent as one photo/video. Videos still go through `process_video` (H.264). No cookies/account required; `data/cookies.txt` remains an optional yt-dlp fallback for other sites.
